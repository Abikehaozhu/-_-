...
def circular_filter(img, filter_r, n=40, filter_thresh=0.4):
    ...
    output = np.where(output < filter_thresh, 0, 255)  # 对图像用阈值分割一下
    end_time = time.time()
...
if __name__ == "__main__":
    ...
    siu, img = cv2.threshold(img, 172, 255, cv2.THRESH_BINARY)  # 二值化图像
    plane = circular_filter(img, 20, n=60, filter_thresh=0.4)
    cv2.imshow("filter_result", plane)
    cv2.waitKey(0)
    cv2.imwrite("result/result_binary.bmp", plane)
